        ...
        try {
            ...
            if ($row = $result->fetch_assoc()) {
                if (password_verify($postData["password"], $row["password"])) {
                    $sqladmin = $this->ask_mysqli->select("admin", $_SESSION["db_1"]) . $this->ask_mysqli->where(array("user" => "admin", "id" => 1), "AND");
                    $resultadmin = $this->adminDB[$_SESSION["db_1"]]->query($sqladmin);
                    $rowadmin = $resultadmin->fetch_assoc();
                    $data = array("message" => "success", "_id" => $row["agent_id"], "user_id" => $row["userid"], "mobile" => $row["mobileno"], "ap" => $row["winper"], "commission" => $row["comission"], "m" => $row["m"],"resultTech" => $rowadmin["resultTech"]);
                } else {
                    $data = null;
                }
            } else {
