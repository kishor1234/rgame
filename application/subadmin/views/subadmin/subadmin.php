...
<script type="text/javascript">
    ...
    function putPoint(id, name, point)
    {
        swal({
            ...
            {
                $.post('<?= api_url ?>/?r=CAddSubAdmin', {id: "<?= $_SESSION["userid"] ?>", userid: id, point: inputValue, action: 'putPointv1'}, function (data) {
                    console.log(data);
                    var json = JSON.parse(data);
                    if (json.status === 1)
                    {
    ...
    function getPoint(id, name, point)
    {
        swal({
            ...
            if ($.isNumeric(inputValue))
            {
                $.post('<?= api_url ?>/?r=CAddSubAdmin', {id: "<?= $_SESSION["userid"] ?>", userid: id, point: inputValue, action: 'getPointv1'}, function (data) {
                    console.log(data);
                    var json = JSON.parse(data);
                    if (json.status === 1)
                    {
