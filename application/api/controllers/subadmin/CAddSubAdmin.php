    ...
    function getPointv1()
    {
        try {
            ...
            if ((float) $row["balance"] >= (float) $data["point"]) {
                ...
                if (empty($error)) {
                    $this->adminDB[$_SESSION["db_1"]]->commit();
                    echo json_encode(array("toast" => array("success", "subadmin", "Information Update Success... "), "status" => 1, "message" => "Agent Information Update Success.. "));
                } else {
                    $this->adminDB[$_SESSION["db_1"]]->rollback();
                    $json_error = json_encode($error);
                    echo json_encode(array("toast" => array("danger", "subadmin", "Information Update Failed... {$json_error}"), "status" => 0, "message" => "Agent Information Update Failed.. {$json_error}"));
                }
            } else {

                echo json_encode(array("toast" => array("danger", "subadmin", "subadmin Insuficent account point"), "status" => 0, "message" => "Agent Insuficent account point"));
            }
        } catch (Exception $ex) {
        }
    }
