...
class CAddUser extends CAaskController
{
    ...
    {
        try {
            $data = array("resultper" => $_POST["resultper"], "winrate" => $_POST["winrate"], "min" => $_POST["min"]);
            if (!isset($_POST["winrate"])) {
                $data = array("resultper" => $_POST["resultper"], "min" => $_POST["min"]);
            }
            $sql = $this->ask_mysqli->update($data, "admin") . $this->ask_mysqli->whereSingle(array("id" => $_POST["id"]));
            if ($this->adminDB[$_SESSION["db_1"]]->query($sql)) {
                echo json_encode(array("toast" => array("success", "Admin", "Information Update Success... "), "status" => 1, "message" => "User Information Update Success.. "));
            } else {
                echo json_encode(array("toast" => array("danger", "Admin", "Information Update Failed... "), "status" => 0, "message" => "User Information Update Failed.. {$json_error}"));
    ...
    function adminBalance()
    {
        ...
        $result2 = $this->adminDB[$_SESSION["db_1"]]->query($sql);
    ...
    {
        unset($_POST["action"]);
        $result = $this->adminDB[$_SESSION["db_1"]]->query($this->ask_mysqli->select("message", $_SESSION["db_1"]) . $this->ask_mysqli->whereSingle(array("id" => $_POST["id"])));
        if ($row = $result->fetch_assoc()) {
